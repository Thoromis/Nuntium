...
class ConversationsAdapter(val startConversation: (data: ConversationItem) -> Unit) : RecyclerView.Adapter<ConversationsAdapter.ConversationsHolder>() {
    var conversationList = ArrayList<ConversationItem>()
        set(value) {
            field = value
            // notify the recyclerview adapter about the changed data
    ...
    override fun onBindViewHolder(holder: ConversationsHolder, position: Int) {
        ...
        holder.textViewConversationPartner.text = currentConversationItem.conversationPartner.firstName.plus(" ").plus(currentConversationItem.conversationPartner.lastName)

        // Load the avatar of the conversation partner
        Glide
